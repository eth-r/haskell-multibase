...
{-|
Decode with the standard set of bases
-}
decode :: ByteString -> Result ByteString
decode = decodeWith standardBases

{-|
Use a specific set of bases to decode from

Can be used to add custom encodings in your application
-}
decodeWith :: M.Map Char Base -> ByteString -> Result ByteString
decodeWith bases bytes =
  decodeWith' bases prefix encoded
  where
    prefix = getPrefix bytes
    encoded = removePrefix bytes

decodeWith' :: M.Map Char Base -> Maybe Char -> Maybe ByteString -> Result ByteString
decodeWith' bases (Just prefix) (Just encoded) = do
  case M.lookup prefix bases of
    Nothing -> Left UnknownBase
    Just base -> baseDecode base encoded
decodeWith' _ _ _ = Left InputTooShort
...
--
-- Inherently suffers from O(n^2) time complexity,
-- not recommended for encoding long bytestrings
base58btc = Base 'z'
  encodeBase58btc
  decodeBase58btc
...
-- | Map of prefix character to the corresponding bases,
-- exported for adding custom bases to the encoding set
standardBases = M.fromList $ map getBasePrefix bases
